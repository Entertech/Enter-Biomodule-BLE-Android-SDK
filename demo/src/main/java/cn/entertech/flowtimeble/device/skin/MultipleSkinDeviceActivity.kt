...
import cn.entertech.ble.function.IDeviceBrainWaveContactFunction
...
import cn.entertech.flowtimeble.device.headband.HandBandFactory
...
class MultipleSkinDeviceActivity : AppCompatActivity() {
...
//            onConnectBound()
        ...
        val disConnectedListener = initMap(disConnectedListenerMap, deviceName) {
            fun(string: String) {
                ...
                runOnUiThread {
                    ...
                    Toast.makeText(
                        this@MultipleSkinDeviceActivity, "$deviceName disconnect ", Toast.LENGTH_SHORT
                    ).show()
                }
            }
        }
    ...
    private fun initPermission() {
        ...
        if (needRequestPermissions.size != 0) {
            ...
            ActivityCompat.requestPermissions(this@MultipleSkinDeviceActivity, permissions, 1)
        }
    }
    ...
    private fun connectDevice(deviceName: String? = "", successUi: (String) -> Unit) {
        ...
        bluetoothDeviceManager?.connectDevice(fun(mac: String) {
            ...
            runOnUiThread {
                Toast.makeText(
                    this@MultipleSkinDeviceActivity,
...
//            startContact(deviceName)
    ...
    private fun notifyBattery(deviceName: String) {
        ...
        (deviceManageMap[deviceName] as? cn.entertech.ble.device.tag.BrainTagManager)?.notifyBattery(
            {
    ...
    private fun notifyBrainData(deviceName: String) {
        ...
        (deviceManageMap[deviceName] as? IDeviceBrainWaveContactFunction<*>)?.notifyBrainWave({
            initMap(this@MultipleSkinDeviceActivity.deviceSaveHelperMap, deviceName) {
                SkinDataHelper(deviceName)
    ...
    private fun stopNotifyBrainData(deviceName: String) {
        ...
        (deviceManageMap[deviceName] as? IDeviceBrainWaveContactFunction<*>)?.stopNotifyBrainWave({
    ...
    private fun startContact(deviceName: String) {
        ...
        if (currentBleManger is IDeviceBrainWaveContactFunction<*>) {
                    ...
                    if (deviceName == deviceFactory?.getDeviceKeyList()?.get(0)) {
                        ...
                        if (count >= 5) {
                            ContextCompat.getColor(
                                this@MultipleSkinDeviceActivity,
                                cn.entertech.base.R.color.color_common_4cd964_ff
                            )
                        } else {
                            ContextCompat.getColor(
                                this@MultipleSkinDeviceActivity,
                                cn.entertech.base.R.color.color_common_ff0000_ff
                            )
                        }
                    )
                }
...
//                showMsg("佩戴检测值： $result")
    ...
    private fun startCollectionReally(bluetoothDeviceManager: BaseBleConnectManager?) {
        when (bluetoothDeviceManager) {
            is ICollectBrainDataFunction -> {
                bluetoothDeviceManager.startCollectBrainData(Unit, success = {
                    showMsg("收集数据指令发送成功 ")
                }) { _, it ->
            ...
            is ICollectBrainAndHrDataFunction -> {
                bluetoothDeviceManager.startCollectBrainAndHrData(Unit, success = {
                    showMsg("收集数据指令发送成功 ")
                })
            }

            is ICollectExerciseDegreeDataFunction -> {
                bluetoothDeviceManager.startCollectExerciseDegreeData(Unit, success = {
                    showMsg("收集数据指令发送成功 ")
                })
            }
        }
    ...
    private fun startCollection(deviceName: String, needStop: Boolean = true) {
        ...
        if (needStop) {
            ...
            stopCollection(deviceName, success = {
                startCollectionReally(bluetoothDeviceManager)
            })
        } else {
    ...
    private fun stopCollection(
        ...
        when (bluetoothDeviceManager) {
            is ICollectBrainDataFunction -> {
                bluetoothDeviceManager.stopCollectBrainData(Unit, success = {
            ...
            is ICollectBrainAndHrDataFunction -> {
                bluetoothDeviceManager.stopCollectBrainAndHrData(Unit, success = {
                    showMsg("$deviceName 停止数据指令发送成功 ")
                    success()
                }) { _, it ->
                    showMsg("$deviceName 停止数据指令发送失败 $it")
